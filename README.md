...
EEGUI MIGRATION
dotnet ef migrations add AddUtenteFigures ^
  --project src/Identity/PrometheonSuite.Identity.Infrastructure ^
  --startup-project src/Identity/PrometheonSuite.Identity.Web ^
...
To update the database use this command from the Web project folder (replace `PrometheonSuite` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../PrometheonSuite.Infrastructure/PrometheonSuite.Infrastructure.csproj -s PrometheonSuite.Web.csproj
```
